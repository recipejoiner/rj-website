...
const AppModal: React.FC<AppModalProps> = ({
  ...
  return (
    ...
    >
      <header className="text-center border-b fixed -mt-10 w-screen pt-2 h-10 bg-white sm:w-80 sm:rounded-t">
        <h3>{modalTitle}</h3>
        {/* X button for closing the modal */}
        <div className="absolute w-full -mt-8">
          <button className="float-right" onClick={() => setModalState(false)}>
            <svg
              className="h-8 w-8 m-1 p-2 text-gray-900 fill-current"
              viewBox="0 0 189 189"
            >
